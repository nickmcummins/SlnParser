...
{
    internal sealed class SolutionFileParser : IParseSolutionSection<IEnumerable<ProjectConfigurationPlatform>>, IParseProjectDefinition
    {
        // c.f.: https://www.codeproject.com/Reference/720512/List-of-Visual-Studio-Project-Type-GUIDs
        private static readonly IDictionary<Guid, ProjectType> _projectTypesByGuid = new Dictionary<Guid, ProjectType>
        { 
        ...
        public IEnumerable<ProjectConfigurationPlatform> Parse(IEnumerable<string> fileContents, string startSection)
        {
        ...
        private static IEnumerable<string> GetFileContentsInSection(IEnumerable<string> fileContents, string startSection)
        {
        ...
        private static ProjectConfigurationPlatform ParseConfigurationPlatform(string line)
        {
            ...
            if (!match.Success)
                throw new UnexpectedSolutionStructureException("Expected to find ConfigurationPlatform but pattern did not match");
        ...
        private static ConfigurationPlatform ParseConfigurationPlatformFromMatch(Match match)
        {
            ...
            var buildConfiguration = Enum.Parse<BuildConfiguration>(buildConfigurationString);
            var buildPlatform = Enum.Parse<BuildPlatform>(buildPlatformString);
            ...
            out ISlnProject project)
        {
            ...
            var projectType = _projectTypesByGuid.TryGetValue(projectTypeGuid, out var projType) ? projType : ProjectType.Unknown;
